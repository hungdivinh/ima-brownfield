...
│ ima-assistant.js│ ───────────► │ Cloudflare Worker  │ ────────────────────► │  Claude API  │
│  (frontend)     │ ◄─────────── │  (proxy + secrets) │ ◄──────────────────── │  (Haiku 4.5) │
└─────────────────┘   reply JSON └────────────────────┘                       └──────────────┘
                                          │ GET (cache 5 phút)
                                          ▼
                                  ┌────────────────────┐
...
- **`worker/index.js`** — proxy giữ `ANTHROPIC_API_KEY` ở env var Cloudflare. Đọc `ima-kb.md` từ GitHub raw, build system prompt, gọi Claude API.
